...
.brand-ruby { color: var(--ruby); font-weight: 700; }
...
@media (max-width: 700px) {
  ...
  .avatar, .avatar-fallback { width: 40px; height: 40px; }
  #header .username { font-size: 14px; }
  #header .id-block { min-width: 0; }
  #header .stats { order: 3; width: 100%; margin-left: 0; justify-content: space-around; gap: 10px; }
  #header .stat .num { font-size: 16px; }
  #header .meta-row { order: 4; }

  /* Stack sidebar above main content, and turn the channel list into a
     horizontally-scrolling strip of chips instead of a tall vertical list —
     the classic mobile pattern for a channel/tab picker. */
  #body { flex-direction: column; }
  #sidebar {
    width: 100%; max-height: none;
    border-right: none; border-bottom: 1px solid var(--border);
  }
  #sidebar .sidebar-title { padding: 10px 14px 4px; }
  #channel-list {
    display: flex; flex-direction: row; flex-wrap: nowrap;
    overflow-x: auto; -webkit-overflow-scrolling: touch;
    gap: 6px; padding: 4px 10px;
  }
  .ch-item { flex: 0 0 auto; margin-bottom: 0; }
  .ch-item .ch-name { white-space: nowrap; }
...
def generate_report_html(
    *,
    ...
    title = f"Ruby — GhostWipe Report — {html_lib.escape(member_name)}"
...
<div id="app">
  ...
  <div id="footer"><span class="brand-ruby">Ruby</span> &middot; GhostWipe v{html_lib.escape(version)} &middot; {html_lib.escape(generated_at or event_time_str)}</div>
</div>
