"""
InterChannel — thin overlay that routes ``send`` / ``typing`` / ``permissions_for``
through the slash-interaction context instead of the regular channel object.
"""
...
class InterChannel(Base):
    """Mixed into the real channel class at runtime so existing code keeps working."""
