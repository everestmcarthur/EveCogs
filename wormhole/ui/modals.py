...
from ..utils import ok_embed, warn_embed
...
class ReportModal(discord.ui.Modal, title="Report Message"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        nets = await self.cog.config.networks()
        nd = nets.get(self.net_name)
        if not nd:
        ...
        report = {
            ...
            "message_content_hash": str(hash(self.message.content or "")),
        ...
        async with self.cog.config.networks() as ns:
            if self.net_name in ns:
                ns[self.net_name].setdefault("reports", []).append(report)
                ns[self.net_name]["report_counter"] = report_id
