...
class ModerationCommands(WormholeBase):
    ...
    async def wh_mod_unban(self, ctx: commands.Context, name: str, user: discord.User) -> None:
        ...
        await self._log(nd, info_embed(f"✅ **{user}** unbanned by {ctx.author}", title="⚙️ Mod Action"))
    ...
    async def wh_mod_unmute(self, ctx: commands.Context, name: str, user: discord.User) -> None:
        ...
        await self._log(nd, info_embed(f"✅ **{user}** unmuted by {ctx.author}", title="⚙️ Mod Action"))
    ...
    async def wh_mod_banserver(self, ctx: commands.Context, name: str, guild_id: int) -> None:
        ...
        await self._log(nd, warn_embed(f"🔨 Server `{guild_id}` banned by {ctx.author}"))
    ...
    async def wh_mod_unbanserver(self, ctx: commands.Context, name: str, guild_id: int) -> None:
        ...
        await self._log(nd, info_embed(f"✅ Server `{guild_id}` unbanned by {ctx.author}", title="⚙️ Mod Action"))
    ...
    async def wh_mod_muteserver(self, ctx: commands.Context, name: str, guild_id: int) -> None:
        ...
        await self._log(nd, warn_embed(f"🔇 Server `{guild_id}` muted by {ctx.author}"))
    ...
    async def wh_mod_allowserver(self, ctx: commands.Context, name: str, guild_id: int) -> None:
        ...
        await self._audit(name, "allowserver", str(ctx.author), str(guild_id))
        await self._log(nd, info_embed(f"Server `{guild_id}` added to allowlist by {ctx.author}", title="⚙️ Mod Action"))
    ...
    async def wh_mod_edit(self, ctx: commands.Context, name: str, message_id: int, *, new_content: str) -> None:
        ...
        await self._log(nd, warn_embed(
            f"✏️ Message `{orig_id}` edited by {ctx.author} ({edited} copies)\n"
            f"New content: {truncate(new_content, 200)}"
        ))
    ...
    async def wh_mod_nuke(self, ctx: commands.Context, name: str, message_id: int) -> None:
        ...
        await self._log(nd, warn_embed(f"💥 Message `{orig_id}` nuked by {ctx.author} ({deleted} copies deleted)"))
